    ...
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="name" lay-verify="required" value="${link.name}" placeholder="请输入名称" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="url"   value="${link.url}" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>

        <div class="layui-form-item" >
            <label class="layui-form-label" style="height: 137px; line-height: 117px">上传图片</label>
            <input name="imgUrl" value="${link.imgUrl}" id="imgurl"  autocomplete="off" class="layui-input" type="hidden">
            <div class=" layui-upload-drag" id="img" >
                <i class="layui-icon">&#xe654;</i>
                <p>点击上传图片</p>
                <img id="demo1" src="${link.imgUrl}" style="position: absolute;height: 137px;width: 237px;margin: -106px auto auto -73px;" />
            </div>
        </div>
...
<script>
    layui.use(['form', 'layedit', 'laydate', 'element', 'layer', 'upload'], function(){
        ...
        form.on('submit(update)', function(data){
            $.post("${basePath}/link/updateChild",data.field,function(data){
                // 获取 session
                if(data.status!=200){
