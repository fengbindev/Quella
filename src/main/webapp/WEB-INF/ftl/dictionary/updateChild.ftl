    ...
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="sequence" value="${dictionary.sequence}" lay-verify="required|number" placeholder="请输入名称" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label"><span class="layui-badge-dot"></span> &nbsp;字典文本</label>
            <div class="layui-input-block">
                <input name="text" value="${dictionary.text}" lay-verify="required" placeholder="请输入字典文本" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label"><span class="layui-badge-dot"></span> &nbsp;字典值</label>
            <div class="layui-input-block">
                <input name="value" value="${dictionary.value}" lay-verify="required" placeholder="请输入字典值" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">描述</label>
            <div class="layui-input-block">
                <input name="summary" value="${dictionary.summary}" placeholder="请输描述" autocomplete="off" class="layui-input" type="text">
            </div>
        </div>
...
<script>
    layui.use(['form', 'layedit', 'laydate', 'element', 'layer', 'upload'], function(){
        ...
        form.on('submit(update)', function(data){
            $.post("${basePath}/dictionary/updateChild",data.field,function(data){
                // 获取 session
                if(data.status!=200){
